...
def mainCode(tractorCheckPosition):
    ...
    if exitCode == 1:
        ...
        return None;
# Globul btw
tractorCheckPosition = (0, 0);
tractorCoordGrid = {}
firstCoordSent = False;
allChecked = False;

while not allChecked:
    tractorCoordGrid[tractorCheckPosition] = mainCode(tractorCheckPosition);
    newPosition = [tractorCheckPosition[0], tractorCheckPosition[1] + 1];
    if newPosition[1] > 49:
        newPosition[0] += 1;
        newPosition[1] = 0;
    if newPosition[0] > 49:
        allChecked = True;
    tractorCheckPosition = tuple(newPosition);

count = 0;
for i in tractorCoordGrid:
    if tractorCoordGrid[i] == "1":
        count += 1;
print(count, len(tractorCoordGrid));
