...
def isOxygenAdjacent(position):
    global robotAreaGrid;
    if robotAreaGrid[position[0], position[1] + 1] == 3 or robotAreaGrid[position[0], position[1] - 1] == 3 or robotAreaGrid[position[0] - 1, position[1]] == 3 or robotAreaGrid[position[0] + 1, position[1]] == 3:
        return True;
    else:
        return False;
...
def recieveInput():
    ...
    if fullyExplored == False:
        ...
        time = 0;
        areaFilled = False;
        while not areaFilled:
            newlyFilledPositions = []
            for i in robotAreaGrid:
                if robotAreaGrid[i] == 1:
                    if isOxygenAdjacent(i):
                        newlyFilledPositions.append(i);
            for i in newlyFilledPositions:
                robotAreaGrid[i] = 3;
            if newlyFilledPositions == []:
                areaFilled = True
                print(time);
            time += 1;
