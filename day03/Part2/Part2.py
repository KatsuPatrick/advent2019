...
def move(currentPath, instruction):
    currentLocation = currentPath[-1];
    if instruction[0] == "U":
        count = int(instruction[1:]);
        j = 0
        while j < count:
            currentPath.append((currentLocation[0], currentLocation[1] + (j + 1)));
            j += 1;
        ...
        j = 0
        while j < count:
            currentPath.append((currentLocation[0] + (j + 1), currentLocation[1]));
            j += 1;
...
def surrounding(currentLocation, wirePoints):
    #returns a set of all surrounding points on the grid that are crossed by the wire (set) provided
    returnSet = set()
    currentUp = (currentLocation[0], currentLocation[1] + 1)
    currentDown = (currentLocation[0], currentLocation[1] - 1)
    currentRight = (currentLocation[0] + 1, currentLocation[1])
    currentLeft = (currentLocation[0] - 1, currentLocation[1])
    if currentUp in wirePoints:
        returnSet.add(currentUp);
    if currentDown in wirePoints:
        returnSet.add(currentDown);
    if currentRight in wirePoints:
        returnSet.add(currentRight);
    if currentLeft in wirePoints:
        returnSet.add(currentLeft);
    return returnSet
...
firstWireCorners = [(0,0)];
secondWireCorners = [(0,0)];

pointer = 0
while pointer < len(firstSanitised):
    firstWirePath = move(firstWirePath, firstSanitised[pointer]);
    firstWireCorners.append(firstWirePath[-1]);
    pointer += 1;

pointer = 0;
while pointer < len(secondSanitised):
    secondWirePath = move(secondWirePath, secondSanitised[pointer]);
    secondWireCorners.append(secondWirePath[-1]);
    pointer += 1;

intersects = set(secondWirePath) & set(firstWirePath);

allWirePoints = set(set(firstWirePath) | set(secondWirePath))

cntr = Counter(firstWirePath);
firstWireDuplicates = set([i for i, v in cntr.items() if v > 1]);
cntr = Counter(secondWirePath);
secondWireDuplicates = set([i for i, v in cntr.items() if v > 1]);

allIntersects = intersects | firstWireDuplicates | secondWireDuplicates;

for oneTwoIntersect in intersects:
    currentDistance = 0;
    currentLocation = oneTwoIntersect;
    if oneTwoIntersect == (0, 0):
        continue;
    previousLocation = None;
    while currentLocation != (0, 0):
        if (currentLocation in allIntersects) == True:
            #choice of direction
            continue;
        else:
            #no choice of direction (unless start)
            if previousLocation == None:
                #starting search
                continue;
